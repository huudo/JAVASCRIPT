...
function AnimateButton(options) {
	...
	if (s == "")
		s = ["matrix(1", "0", "0", "1", "0", "0)"].join(G_COMMA);
	var _a = s.split(G_COMMA);
	var zoom = parseFloat(_a[3]); // use zoomY as zoom unit.
	var zoomX = parseFloat(_a[0].split("(")[1]); // "matrix(x" ==> x
	_zoomMin *= zoom;
	_zoomMax *= zoom;
	_zoomStep *= zoom;
	_zoomStepX = _zoomStep * zoomX / zoom;
	//		console.log("id, zoomStep, zoomStepX", _id, _zoomStep, _zoomStepX);
	...
	function zoomIn(x) {
		setTimeout(function() {
			if (_mode != ZOOM_IN)
				return;
			var a = x.attr("transform").split(G_COMMA);
			var dY = parseFloat(a[3]);
			if (dY >= _zoomMax) {
				_mode = NORMAL;
				return;
			}
			dY += _zoomStep;
			var dX = parseFloat(a[0].split("(")[1]);
			dX += _zoomStepX;
			x.attr("transform", "matrix(" + [dX, 0, 0, dY, a[4], a[5]].join(G_COMMA));
	...
	$(document.getElementById(_id)).mousedown(function(event) {
		...
		if (parseFloat(_e.attr("transform").split(G_COMMA)[3]) >= _zoomMax)
			return;
		...
		if (parseFloat(_e.attr("transform").split(G_COMMA)[3]) <= _zoomMin)
			return;
		_mode = ZOOM_OUT;
		zoomOut(_e);
